...
export default function ProductCard({ product }) {
  async function addRecents() {
    ...
    const response = await fetch(
      `http://ec2-13-203-205-26.ap-south-1.compute.amazonaws.com:8081/product/recent?product_id=${product.id}`,
      {
        method: 'POST',
        headers: {
  ...
  return (
    ...
    >
      ...
      <div className="rating21">
        ...
        <div>
          <StarIcon fontSize="smaller" style={{ color: 'green' }} />|{' '}
          {product?.rating?.rate_count}{' '}
        </div>
      </div>
