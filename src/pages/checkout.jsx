...
export default function Checkout(){
  ...
  async function placeOrder() {
    ...
    }),fetch(`http://localhost:8080/order/${cartId}?address_id=${selectAddressId}`,{
        method:'POST',
        credentials:'include',
      headers:{
        'Content-Type':'application/json',
      ...
      <div className="check-out-parent">
        <div className="check-out-cont">
          <div className="login-check">
            <div style={{width:'8%',paddingTop:'1.6%',color:'rgb(203, 203, 203)'}}>
               <LooksOneIcon style={{fontSize:'180%'}}/>
            </div>
            <div style={{width:'70%',display:'flex',flexDirection:'column',justifyContent:'center'}}>
              <div style={{height:'40%',textAlign:'left',fontSize:'110%',color:'gray',display:'flex'}}>
                 <div style={{lineHeight:'150%'}}>LOGIN</div>
                 <div style={{lineHeight:'10%',color:'#2874f0'}}>
                 <DoneIcon style={{fontSize:'120%'}}/>
                 </div>
              </div>
              <div style={{height:'30%',textTransform:'capitalize',textAlign:'left',fontWeight:'550'}}>
                {username} {' '} <span style={{fontWeight:'200',fontSize:'80%'}}>+91{selectedAddress?.phone}</span>
              </div>
            </div>
            
          </div>
          <div className="check-address">
          <div style={{width:'8%',paddingTop:'2%',color:'rgb(203, 203, 203)'}}>
               <LooksTwoIcon style={{fontSize:'180%'}}/>
            </div>
            <div style={{width:'65%',display:'flex',flexDirection:'column',justifyContent:'center'}}>
              <div style={{height:'30%',textAlign:'left',fontSize:'110%',color:'gray',display:'flex'}}>
              <div style={{lineHeight:'150%'}}>DELIVERY ADDRESS</div>
                 <div style={{lineHeight:'10%',color:'#2874f0'}}>
                 <DoneIcon style={{fontSize:'120%'}}/>
                 </div>
              </div>
              <div style={{height:'40%',width:'90%',textTransform:'capitalize',textAlign:'left',fontSize:'80%',fontWeight:'lighter'}}>
                <span style={{fontSize:'120%',fontWeight:'550'}}>{selectedAddress?.fullName}</span>{' '}{' '}{selectedAddress?.flatNumber}, {selectedAddress?.area}, {selectedAddress?.village}, {selectedAddress?.district}, <span>{selectedAddress?.village}</span>, {selectedAddress?.state} - <span style={{fontSize:'120%',fontWeight:'550'}}>{selectedAddress?.pinCode}</span>
              </div>
            </div>
          ...
          <div className="checkout-pro">
            <div style={{height:'2.5rem',display:'flex',backgroundColor:'#2874f0',color:'white'}}>
        ...
        <button className="place-order-btn " onClick={placeOrder}> Continue</button>
         </div>
      </div>
    ) 
}